...
class MLBacktester():
    ...
    def __init__(self, symbol, start, end, tc=0, granularity='M1'):
        '''
        Parameters
        ----------
        symbol: str
    ...
    def plot_results(self):
        ...
        if self.results is None:
            ...
            title = f"Logistic Regression: {self.symbol} | TC = {self.tc} | Lags = {self.lags}"
...
print(f'Best: {best} lag(s) | Results: {results}')
print(ml.results.trades.value_counts())

print(ml.test_strategy(train_ratio=0.7, lags=best))
ml.plot_results()
plt.show()
print(ml.results)

pickle.dump(ml.model, open("logreg.pkl", "wb"))
pickle.dump(ml.lags, open("lags.pkl", "wb"))
pickle.dump(ml.granularity, open("granularity.pkl", "wb"))
